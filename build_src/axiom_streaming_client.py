...
# ── Error telemetry ──
_CLIENT_VERSION = "6.39"
_TELEMETRY_URL = "https://axiom.heliex.net/error_report.php"

def _report_error(experiment_name, error_type, error_message, platform=None, gpu_name=None, gpu_memory_mb=0):
    """Fire-and-forget error report to server. Never raises."""
    try:
        import urllib.request, ssl, platform as _pf
        if platform is None:
            platform = _pf.system() + "_" + _pf.machine()
        payload = json.dumps({
            "experiment_name": str(experiment_name)[:256],
            "error_type": str(error_type)[:256],
            "error_message": str(error_message)[:2000],
            "platform": str(platform)[:128],
            "client_version": _CLIENT_VERSION,
            "has_gpu": HAS_GPU,
            "gpu_name": str(gpu_name or "")[:256],
        ...
        req = urllib.request.Request(
            _TELEMETRY_URL,
            data=payload,
            headers={"Content-Type": "application/json", "User-Agent": "AxiomBOINC/" + _CLIENT_VERSION},
            method="POST",
        )
        urllib.request.urlopen(req, timeout=5, context=ctx)
        print("[TELEMETRY] Error reported")
    except Exception:
        pass  # Never let telemetry crash the client
...
class HebbianTrainer:
    ...
    async def initialize(self):
        ...
        if self.sector_mode:
            ...
            if HAS_GPU:
                import cupy
                free_mem = cupy.cuda.Device().mem_info[0]
...
def _run_experiment_mode(wu, result_file):
    ...
    # Derive deterministic seed from experiment name (includes unique hex suffix)
    import hashlib as _hl
    exp_seed = int(_hl.md5(experiment_name.encode()).hexdigest()[:8], 16) % (2**31) if experiment_name != 'unknown' else 42
    ...
    try:
        ...
        req = urllib.request.Request(script_url, headers={'User-Agent': 'AxiomBOINC/6.39'})
        ...
        _report_error(experiment_name, type(e).__name__, f"Script download failed: {e}")
        with open(result_file, 'w') as f:
    ...
    try:
        ...
        try:
            ...
            _cur_dev = cupy.cuda.Device().id
            gpu_name = cupy.cuda.runtime.getDeviceProperties(_cur_dev)['name'].decode()
            gpu_mem = cupy.cuda.runtime.getDeviceProperties(_cur_dev)['totalGlobalMem']
        ...
        exec_namespace['WU_NAME'] = experiment_name
        ...
        _report_error(experiment_name, type(e).__name__, str(e))
...
def run_boinc_mode(wu_file, result_file):
    ...
    # Read gpu_device_num from init_data.xml (BOINC assigns different GPUs to different slots)
    _gpu_dev = 0
    try:
        _init_data_path = os.path.join(os.getcwd(), 'init_data.xml')
        if os.path.exists(_init_data_path):
            with open(_init_data_path, 'r') as _idf:
                _init_xml = _idf.read()
            _m = re.search(r'<gpu_device_num>(\d+)</gpu_device_num>', _init_xml)
            if _m:
                _gpu_dev = int(_m.group(1))
                print(f"[BOINC] init_data.xml gpu_device_num={_gpu_dev}")
    except Exception as _e:
        print(f"[BOINC] Could not read gpu_device_num: {_e}")
    if HAS_GPU:
        import cupy
        if _gpu_dev >= 0:
            cupy.cuda.Device(_gpu_dev).use()
            print(f"[BOINC] Set CUDA device to GPU {_gpu_dev}")
        _mem = cupy.cuda.Device(_gpu_dev).mem_info
    ...
    if wu is None:
        ...
        _report_error("unknown", "WUReadError", f"Could not read work unit: {wu_file}")
        with open(result_file, 'w') as f:
    ...
    if wu.get('mode') == 'experiment' or wu.get('script_url'):
        ...
        try:
            ...
            import traceback; traceback.print_exc()
            _report_error(wu.get('experiment_name', 'unknown'), type(_exp_err).__name__, str(_exp_err))
            try:
    ...
    _report_error(wu.get('experiment_name', 'unknown'), 'UnrecognizedMode', f'mode={mode}')
